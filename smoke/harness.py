...
LE_PATTERNS = [
    ("이하", _AFTER_NUM + r"이하|^이하"), ("미만", _AFTER_NUM + r"미만|^미만"), ("아래", r"(?<![가-힣])아래(?!로\s*(?:안|못)\s)"), ("밑", r"(?<![가-힣])밑(?!돌지\s*않)(?!으로\s*(?:안|못)\s)"),
    ...
    # 부정형("~하지 않으면")은 반대 방향이다. "넘지 않으면" = 그 아래 유지 = LE
    ("넘지않", r"넘지\s*않|넘기지\s*않"), ("초과안", r"초과하지\s*않"), ("상회안", r"상회하지\s*않"),
    ("돌파안", r"돌파하지\s*않"), ("오르지않", r"오르지\s*않|올라[가오]지\s*않"),
    ("위로안", r"위로\s*(?:안|못)\s+(?:올라|가|넘)"),
]
GE_PATTERNS = [
    ("이상", _AFTER_NUM + r"이상|^이상"), ("초과", _AFTER_NUM + r"초과(?!하지\s*않)|^초과(?!하지\s*않)"), ("위로", r"(?<![가-힣])위(?:로(?!\s*(?:안|못)\s)|에서|면|쪽)"),
    ...
    # 부정형("~하지 않으면")은 반대 방향이다. "밑돌지 않으면" = 그 위 유지 = GE
    ("밑돌지않", r"밑돌지\s*않"), ("떨어지지않", r"떨어지지\s*않|빠지지\s*않"),
    ("내려가지않", r"내려[가오]지\s*않"), ("하회안", r"하회하지\s*않"),
    ("깨지않", r"깨지\s*않|무너지지\s*않"),
